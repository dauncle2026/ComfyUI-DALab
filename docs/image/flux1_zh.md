...
### 批量生成：搭配 Qwen LLM
利用 **DA Qwen LLM** 节点批量生成创意提示词，实现自动化连续生成。
...
| 参数名 | 默认值 | 说明 |
| :--- | :--- | :--- |
| clip_model | CLIP L | CLIP 文本编码器，通常使用 clip_l。 |
| t5_model | T5 (FP8) | T5 文本编码器，推荐使用 FP8 版本以节省显存。 |
| vae_model | Flux VAE | 图像解码 VAE 模型。 |
| diffusion_model | Flux Dev (FP8) | Flux 核心扩散模型 (Transformer)。 |
