...
### 1. 初始化配置（仅需一次）
1. 添加 DA Flux1 Config 节点。
2. 选择对应的模型文件，调整采样参数（如步数、CFG 等）。
3. 运行一次 (Queue Prompt)。此时配置信息会自动保存到本地。
4. *注：配置完成后，您可以删除或禁用 Config 节点，直到需要修改参数为止。*

<img src="../assets/flux1_config.jpg" width="80%" />

### 2. 日常生成
1. 只需保留 DA Flux1 节点。
2. 在 Prompts 输入框中输入提示词。
3. 点击 Queue Prompt 即可生成。节点会自动读取之前保存的配置。
