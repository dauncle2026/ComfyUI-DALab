...
try:
    ...
    # uncle da tools compatibility with older transformers without QuantizedCacheConfig
    class QuantizedCacheConfig(dict):
        def __init__(self, backend="quanto", nbits=4, **kwargs):
            super().__init__(backend=backend, nbits=nbits, **kwargs)
...
try:
    from transformers.generation.logits_process import _crop_past_key_values
except Exception:
    # uncle da tools compatibility with older transformers without _crop_past_key_values
...
try:
    ...
    # uncle da tools compatibility with older transformers without NEED_SETUP_CACHE_CLASSES_MAPPING and QUANT_BACKEND_CLASSES_MAPPING
