...
class IndexTTS2:
    def __init__(
        ...
        self.qwen_emo = QwenEmotion(
            qwen_emo_path,device=self.device
        )
        ...
        w2v_path = os.path.join(self.model_dir, "w2v-bert-2.0")
        w2v_stat_path = os.path.join(self.model_dir, self.cfg.w2v_stat)
        # wav to audio feature
        self.extract_features = SeamlessM4TFeatureExtractor.from_pretrained(w2v_path)
        # audio feature to vector
        ...
        # continue vector to vector token
        ...
        # wav to style
        campplus_ckpt_path = os.path.join(self.model_dir, "funasr-campplus","campplus_cn_common.bin")
        campplus_model = CAMPPlus(feat_dim=80, embedding_size=192)
        campplus_model.load_state_dict(torch.load(campplus_ckpt_path, map_location="cpu"))
        self.campplus_model = campplus_model.to(self.device)
        self.campplus_model.eval()

        # audio feature to audio
        bigvgan_path = os.path.join(self.model_dir, "bigvgan")
        ...
        # 6 emotion style matrix
        emo_matrix = torch.load(os.path.join(self.model_dir, self.cfg.emo_matrix))
        self.emo_matrix = emo_matrix.to(self.device)
        self.emo_num = list(self.cfg.emo_num)

        # speaker style matrix
    ...
    def infer(self, spk_audio_prompt, text, output_path,
              ...
              verbose=False, max_text_tokens_per_segment=120, stream_return=False, more_segment_before=0, speed=1.0, **generation_kwargs):
        if stream_return:
            return self.infer_generator(
                ...
                verbose, max_text_tokens_per_segment, stream_return, more_segment_before, speed, **generation_kwargs
            )
        else:
            try:
    ...
    def infer_generator(self, spk_audio_prompt, text, output_path,
              ...
              emo_vector=None,
              use_emo_text=False, emo_text=None, use_random=False, interval_silence=200,
              verbose=False, max_text_tokens_per_segment=120, stream_return=False, quick_streaming_tokens=0,speed=1.0, **generation_kwargs):
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast(text_tokens.device.type, enabled=dtype is not None, dtype=dtype):
                    ...
                    base_scale = 1.72
                    target_lengths = (code_lens * (base_scale / speed)).long()
...
class QwenEmotion:
    def __init__(self, model_dir, device="cuda"):
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            self.model_dir,
            dtype="float16",  # "uncle da tools compatibility with older transformers without torch_dtype"
            device_map=device
        )
        self.prompt = "文本情感分类"
        self.cn_key_to_en = {
