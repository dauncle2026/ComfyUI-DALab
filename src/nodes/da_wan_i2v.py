...
class DAWanI2VConfig(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        ...
        return io.Schema(
            node_id="DAWanI2VConfig",
            display_name="DA Wan2.2 I2V Config",
            category="DALab/Video/Wan2.2 I2V",
            description="Configure the Wan2.2 I2V model params. Run first to save config.",
            is_output_node=True,
            inputs=[
                io.Combo.Input(
                    "text_encoder_model",
                    ...
                    tooltip="The Wan Text Encoder. Default: models/text_encoders/wan/umt5_xxl_fp8_e4m3fn_scaled.safetensors",
                ),
                io.Combo.Input(
                    "vae_model",
                ...
                io.Combo.Input(
                    "high_model",
                    default=config.get("high_model", "wan2.2/wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors"),
                    options=diffusion_options,
                    display_name="high_model",
                    tooltip="The Wan2.2 High Noise Diffusion Model (14B) for I2V.",
                ),
                io.Combo.Input(
                    "low_model",
                    default=config.get("low_model", "wan2.2/wan2.2_i2v_low_noise_14B_fp8_scaled.safetensors"),
                    options=diffusion_options,
                    display_name="low_model",
                    tooltip="The Wan2.2 Low Noise Diffusion Model (14B) for I2V.",
                ),
                io.Int.Input(
                    "steps",
...
class DAWanI2V(io.ComfyNode):
    @classmethod
    def reset_cache(cls):
        _CACHE.positive = {}
        _CACHE.negative = {}
        _CACHE.concat_latents = {}

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="DAWanI2V",
            display_name="DA Wan2.2 I2V",
            category="DALab/Video/Wan2.2 I2V",
            description="Generate videos using the Wan2.2 I2V model.",
            is_input_list=True,
            inputs=[
                io.Int.Input(
                    "width",
    ...
    def execute(
        ...
        text_encoder_path = folder_paths.get_full_path_or_raise("text_encoders", config.get("text_encoder_model"))
        vae_path = folder_paths.get_full_path_or_raise("vae", config.get("vae_model"))
        high_model_path = folder_paths.get_full_path_or_raise("diffusion_models", config.get("high_model"))
        low_model_path = folder_paths.get_full_path_or_raise("diffusion_models", config.get("low_model"))
        ...
        try:
            ...
            for idx, batch_input in enumerate(batch_inputs):
                ...
                if prompt["cache_key"] in _CACHE.positive:
                    logger.info(f"WanI2V positive prompt cache hit: {prompt['cache_key']}")
                ...
                all_conditionings.append((positive, negative, frame_cnt, first_frame, last_frame))

            vae = manager.get_vae(vae_path)

            all_encoded_frames = []
            for positive, negative, frame_cnt, first_frame, last_frame in all_conditionings:
                latent_image = torch.zeros(
                    [1, 16, ((frame_cnt - 1) // 4) + 1, latent_height, latent_width],
                    device="cpu"
                )

                init_image = torch.ones((frame_cnt, aligned_height, aligned_width, 3)) * 0.5
                init_mask = torch.ones((1, 1, latent_image.shape[2] * 4, latent_image.shape[-2], latent_image.shape[-1]))

                concat_latent_image, concat_latent_mask = cls.encode_frame(
                    vae,
                    init_image,
                    init_mask,
                    first_frame,
                ...
                positive_with_concat = node_helpers.conditioning_set_values(
                    positive, {"concat_latent_image": concat_latent_image, "concat_mask": concat_latent_mask}
                )

                all_encoded_frames.append((positive_with_concat, negative, frame_cnt, latent_image))
            ...
            all_high_samples = []
            for idx, (positive, negative, frame_cnt, latent_image) in enumerate(all_encoded_frames):
                logger.info(f"High diffusion processing {idx+1}/{len(all_encoded_frames)}")
            ...
            for idx, (low_samples, frame_cnt) in enumerate(all_low_samples):
                ...
                if images.shape[1] != target_height or images.shape[2] != target_width:
                    images = utils.scale_by_width_height(images, target_width, target_height, "bilinear", "center")
    ...
    def encode_frame(
        ...
        total_frames = init_image.shape[0]
        ...
        cache_parts = [
            ...
            f"w_{width}",
            f"h_{height}",
            first_frame["cache_key"] if first_frame["value"] is not None else "Empty",
            last_frame["cache_key"] if last_frame["value"] is not None else "Empty"
        ]
        cache_key = "_".join(cache_parts)

        if cache_key in _CACHE.concat_latents:
            logger.info(f"WanI2V concat latent image cache hit: {cache_key}")
            concat_latent_image = _CACHE.concat_latents[cache_key]['image']
            concat_latent_mask = _CACHE.concat_latents[cache_key]['mask']
        else:
            if first_frame["value"] is not None:
                ...
                scaled_first_frame = utils.scale_by_width_height(
                    first, init_image.shape[2], init_image.shape[1], "area", "center"
                )

                init_image[:scaled_first_frame.shape[0]] = scaled_first_frame
                init_mask[:, :, :scaled_first_frame.shape[0] + 3] = 0.0
            ...
            if last_frame["value"] is not None:
                ...
                scaled_last_frame = utils.scale_by_width_height(
                    last, init_image.shape[2], init_image.shape[1], "area", "center"
                )

                init_image[-scaled_last_frame.shape[0]:] = scaled_last_frame
                init_mask[:, :, -scaled_last_frame.shape[0]:] = 0.0

            concat_latent_image = vae.encode(init_image[:, :, :, :3])
            concat_latent_mask = init_mask.view(
                1, init_mask.shape[2] // 4, 4, init_mask.shape[3], init_mask.shape[4]
            ).transpose(1, 2)

            _CACHE.concat_latents[cache_key] = {
